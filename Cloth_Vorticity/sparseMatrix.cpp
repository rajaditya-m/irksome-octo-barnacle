...
void SparseMatrix::Add3x3Entry(int row, int denseCol, double *data, double scalarMult) {
  for(int c=0;c<3;c++) {
    for(int r=0;r<3;r++) {
      int invColIdx = GetInverseIndex(row+r,denseCol+c);
      AddEntry(row+r,invColIdx,data[(c*3)+r]*scalarMult);
    }
  }
}
